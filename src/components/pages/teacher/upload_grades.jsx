    ...
    <div className="relative min-h-screen h-screen flex font-sans bg-gray-100 dark:bg-black/90bg-gray-100 overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          />
          <h2
            className={`text-center dark:text-teal-400 font-bold text-lg ${!isSidebarOpen ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}
          >
            Panel Docente
          </h2>
        </div>
        ...
        >
          <ChevronLeftIcon
            className={`size-5 ${isSidebarOpen ? "rotate-0" : "rotate-180"} transition`}
          />
        </button>
        ...
        <nav className="flex flex-col gap-2 h-auto">
          {menuItems.map((item) => (
            <NavLink
              ...
              to={item.link}
              className={({ isActive }) =>
                `flex items-center gap-2 px-3 py-2 w-full rounded-lg  dark:hover:text-white dark:hover:bg-white/5 transition-all hover:text-white hover:bg-red-500 text-black dark:text-white font-bold
                ${isActive ? "dark:!bg-teal-700 bg-red-700 text-white font-bold" : ""}
                `
              }
            >
        ...
        <div className="mt-auto border-t border-gray-700 pt-4">
          <button
            onClick={logOut}
            className="w-full flex justify-center items-center gap-2 px-3 py-2 rounded-lg  hover:bg-red-400/20   hover:text-red-400 hover:bg-red-300 dark:hover:bg-orange-900transition-all"
          >
